...
function Card({ review, dim }: { review: Review; dim: boolean }) {
  return (
    <div
      style={{
        position: "relative",
...
function FeedbackDoodle({ sectionRef }: { sectionRef: React.RefObject<HTMLElement | null> }) {
  ...
  return (
    <div aria-hidden style={{ position: "absolute", inset: 0, overflow: "hidden", zIndex: 0, pointerEvents: "none" }}>
      <svg viewBox="0 0 1000 2400" preserveAspectRatio="xMidYMid slice" style={{ position: "absolute", inset: 0, width: "100%", height: "100%" }}>
        <path d={d} fill="none" stroke={stroke} strokeOpacity="0.06" strokeWidth="90" strokeLinecap="round" strokeLinejoin="round" />
        <motion.path d={d} fill="none" stroke={stroke} strokeOpacity="0.18" strokeWidth="90" strokeLinecap="round" strokeLinejoin="round" style={{ pathLength }} />
      </svg>
    </div>
  );
}
